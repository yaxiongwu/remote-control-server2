	...
	if(speed >=0){
	 if(speed>8) speed=8;
	   ...
	   {	     
	     ...
	     pwmWrite(pwm0_0,128*speed);    
	     pwmWrite(pwm1_0,128*speed);
	  ...
	  if(lastSpeed <0) //speed<0 and lastSpeed<0
	   {	     
	     ...
	     pwmWrite(pwm0_1,-128*speed);    
	     pwmWrite(pwm1_1,-128*speed);    
	   }else{ //speed<0 and lastSpeed>=0
...
void directionControl(int lastSpeed,int direction){	
    ...
    switch (lastSpeed){
	  ...
	  pwmWrite(pwm0_0,-128*tempSpeed);
	 }else{  //turn left
	  tempSpeed=lastSpeed-direction;
	  if(tempSpeed<-8) tempSpeed=-8;
	  pwmWrite(pwm1_0,-128*tempSpeed);
	 }
       break;
      case -4:
      case -3:
