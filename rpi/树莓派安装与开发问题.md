...
```
   ...
   如果断开HDMI，自动运行程序时，没有了HDMI，1变成了USB 录音设备，无法播放声音，会导致无法打开的错误，而是程序崩溃。
   所以没有HDMI时应该设置sudo amixer cset numid=3 0
   
   命令sudo alsamixer 设置音量
   命令speaker-test  -t3 测试声音输出
   
   如果用硬件pwm，12脚gpio 1为PCM_Clk和PWM0复用，PCM_Clk为音频时钟，所以要用pwm就不能用耳机孔输出音频，只能用蓝牙或hdmi音频分离
   
   另外使用sudo 设置的root的参数，没有sudo设置的是pi的参数，两者不同不相干
```
## 八、下载程序
```
